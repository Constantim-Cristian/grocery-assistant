...
The data acquisition module is engineered for resilient and precise data extraction from dynamic API endpoints within the Wolt platform, targeting six prominent Romanian grocery retailers: Freshful, Profi, Penny, Auchan, Carrefour, and Kaufland.

Robust API Interaction: Employs sophisticated retry mechanisms and exponential backoff to effectively manage API rate limits, network transients, and ensure data integrity during collection. This demonstrates a professional approach to handling external service dependencies.

Advanced Data Extraction & Normalization: Utilizes regular expressions for complex pattern matching and extraction of critical product attributes (e.g., quantities like "6x0.33L", "100g") from varied and often inconsistent product titles. This includes dedicated functions for data cleaning and standardization.

Unit Standardization & Price-per-Unit Calculation: Implements custom logic to standardize diverse measurement units (e.g., converting 'g' to 'kg', 'ml' to 'l') and accurately calculate the crucial "price per unit of measure," enabling genuine value comparison across different product sizes and packaging.
...
Dynamic Category Mapping: Features a structured approach for rule-based categorization, demonstrating a systematic method for semantic data organization.

Interactive Dashboard Module
The interactive dashboard module provides a highly interactive and responsive user interface for exploring and optimizing grocery purchases based on the locally collected data.

Stateful UI Management: Leverages advanced UI state management techniques for persistent management of user selections, filters, and shopping cart contents across application reruns, ensuring a seamless and intuitive user experience.
...
Unique Item Tracking: Utilizes a system for assigning unique identifiers to each product, enabling precise tracking and manipulation of items within the dynamic shopping cart, preventing ambiguity and ensuring accurate quantity adjustments and price calculations.
...
This project is architected as a strong foundation for future enhancements involving Artificial Intelligence and Machine Learning:
